...
class Scanning extends Component
{
    ...
    {
        return view('livewire.scanning')->layout('layouts.scan', ['title' => 'Presensi Yayasan Ariya Metta']);
    }
    ...
    private function proses_absen_pd(){
        ...
        if($jam_pd){
            ...
            $this->toastr('error', 'Absen Gagal', 'Setting JAM tidak ditemukan!', 'Dong.mp3');
        }
        ...
        if($absen_masuk){
            ...
            if($absen_pulang){
                ...
                $absen_pulang = Absen_pulang::updateOrCreate(
                    ...
                    [
                        'pulang_cepat' => (Str::contains($from->diffInMinutes($to, false), '-')) ? 0 : $from->diffInMinutes($to, false),
    ...
    public function scanMasukGuru($data){
        //dd($data);
    }
    public function scanMasukSiswa($data){
        //dd($data);
    }
}
